...
def Flowers():
    Sports = 'Baseball Rugby Soccer Debates Starcraft2'.split()
    Lawns = 'Cleanly-Cut-Grass Astroturf Flowers Trees Rocks'.split()
    Transportations = 'Walking Motorcycle Bicycle Bus Car'.split()
    Names = 'Albert Dietfried Virgilijus Gallchobhar Bricius'.split()
    variables = Sports + Lawns + Transportations + Names
    domains = {}
    for var in variables:
        domains[var] = list(range(1, 6))
    domains['Debates'] = [2]
    domains['Bicycle'] = [4]
    neighbors = parse_neighbors("""Ablert: Baseball;
            Dietfried: Cleanly-Cut-Grass; Virgilijus: Rugby;
            Gallchobhar: Walking; Bricius: Virgilijus;
            Astroturf: Baseball; Flowers: Trees;
            Flowers: Rocks; Baseball: Motorcycle; 
            Soccer: Bus; Trees: Car; Gallchobhar: Rugby""")
    for type in [Sports, Lawns, Transportations, Names]:
        for A in type:
            for B in type:
                if A != B:
    ...
    def flowers_constraint(A, a, B, b, recurse=0):
        same = (a == b)
        next_to = abs(a - b) == 1
        if A == 'Albert' and B == 'Baseball':
            return same
        if A == 'Dietfried' and B == 'Cleanly-Cut-Grass':
            return same
        ...
        if A == 'Gallchobhar' and B == 'Rugby':
            return a - 1 == b
        if recurse == 0:
            return flowers_constraint(B, b, A, a, 1)
        if ((A in Sports and B in Sports) or
                (A in Lawns and B in Lawns) or
                (A in Transportations and B in Transportations) or
                (A in Names and B in Names)):
            return not same
        raise Exception('error')
    return CSP(variables, domains, neighbors, flowers_constraint)
...
def solve_flowers(algorithm=min_conflicts, **args):
    f = Flowers()
    ans = algorithm(f, **args)
    for h in range(1, 6):
        print('House', h, end=' ')
        for (var, val) in ans.items():
            if val == h:
                print(var, end=' ')
        print()
    return ans['Flowers'], ans['Starcraft2'], f.nassigns, ans

#solve_zebra(backtracking_search)
solve_flowers(backtracking_search)